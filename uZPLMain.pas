...
uses
  ...
  frxExportPDF, FireDAC.Phys.OracleDef, Vcl.OleCtrls, SHDocVw, PythonEngine;
...
type
  ...
  private
    ...
    FPDFExportPath: String;
    FModifiedPDFPath: String;
    procedure LoadIni();
    procedure SaveIni();
  protected
...
begin
  ...
  try
    ...
    begin
      ...
      LPDFName.Caption := ExtractFileName(InputPDFPath);
      LPDFName.Visible := True;
...
begin
  ...
  // Check if the PDF exists and open it in the WebBrowser
  if FileExists(OutputPDF) then
  begin
    WebBrowser1.Navigate('file://' + OutputPDF);  // Display PDF within the application
    ShellExecute(0, 'open', PChar(OutputPDF), nil, nil, SW_SHOWNORMAL);  // Open PDF externally
  end
  else
...
begin
  ...
  FDConnOracle2.LoginPrompt := False;

  try
    FDConnOracle2.Connected := True;
...
begin
  ...
  try
    ...
    begin
      ...
      FPDFExportPath := ChangeFileExt(FReportFileName, '.pdf');
